...
<script type="text/javascript" src="../resources/js/affiliation/inspection-by-scrutiny-committee.js"></script>
...
<%
//According tab list
String Insp_remarks="",Insp_recm="",inspection_id="",memberfile="",inspection_by_id="";
...
inspection_by_id=  General.checknull(request.getParameter("inspection_by_id"));

String status=  General.checknull(request.getParameter("status"));
String remarks=  General.checknull(request.getParameter("remarks"));
String is_finalsubmit_open=  General.checknull(request.getParameter("is_finalsubmit_open"));
...
%>	
   ...
   <div id="accordionNine" class="accordion">
		...
		<div class="form-group ">
			<div class="row">
				<div class="col-sm-12">
					<label for="" class="col-sm-2 col-form-label required-field"><b>Status</b></label>
						<div class="col-sm-4">
						<%if(!status.equals("RO") || is_finalsubmit_open.equals("Y")){ %>
							<select class="form-control" id="insp_status" name="insp_status" 
							<%if(status.equals("Y") || status.equals("N") || (status.equals("RO") && !is_finalsubmit_open.equals("N") )){ %>disabled<%} %>>
								<option value="">Select Status</option>
								<option value="Y" <%if(status.equals("Y")){ %>selected<%} %>>Approve</option>
								<option value="N" <%if(status.equals("N")){ %>selected<%} %> >Reject</option>
								<option value="RO" <%if(status.equals("RO")){ %>selected<%} %>>Re-open</option>
							</select>
							<%} %>
							<%if(status.equals("RO") && is_finalsubmit_open.equals("N")){ %>
							<select class="form-control" id="insp_status" name="insp_status" 
							<%if(status.equals("Y") || status.equals("N") || (status.equals("RO") && !is_finalsubmit_open.equals("N") )){ %>disabled<%} %>>
								<option value="">Select Status</option>
								<option value="Y" <%if(status.equals("Y")){ %>selected<%} %>>Approve</option>
					...
					<label for="" class="col-sm-2 col-form-label required-field"><b>Remarks</b></label>
						<div class="col-sm-4">
							<textarea class="form-control" id="insp_remarks" maxlength="300" style="resize:vertical;max-height:120px;height:40px;" placeholder="Enter Remarks" <%if(status.equals("Y") || status.equals("N") || status.equals("RO")){ %>disabled<%} %>><%=remarks%></textarea>
						</div>
				</div>
			</div>
		</div>
					...
					if(!(status.equals("Y") || status.equals("N") || status.equals("RO"))){ %>
                		<button type="button" class="btn btn-view" id="btnSave" name="btnSave" value="SAVE" onclick="save('U', '<%=Inst_Id%>')">Update Status</button>
                	<%}else if(status.equals("RO")){
                		if(is_finalsubmit_open.equals("N")){ %>
               			<div class="errmessage2" id="errMsg2">Re-open application was not submitted with in the extended date.</div>
                		<button type="button" class="btn btn-view" id="btnSave1" name="btnSave" value="SAVE" onclick="save('U', '<%=Inst_Id%>')">Update Status</button>
                			<%}else{ %>
                		<div class="errmessage1" id="errMsg1">Application Re-open for 3 days & acknowledgement shared to college</div>	
                			<%} %>
                	<%} %>
                	<button type="button" class="btn btn-view" id="btnReset" name="btnReset">Reset</button>
